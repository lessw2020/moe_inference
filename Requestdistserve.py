...
from distserve.config import ParallelConfig
...
class SamplingParams:
    ...
    def _validate_basic_params(self) -> None:
        ...
        if self.best_of < self.n:
            raise ValueError(f"best_of must be >= n, got n={self.n}, best_of={self.best_of}")
        
        for penalty in [self.presence_penalty, self.frequency_penalty]:
            if not (self._PENALTY_RANGE[0] <= penalty <= self._PENALTY_RANGE[1]):
                raise ValueError(f"Penalty must be in [{self._PENALTY_RANGE[0]}, {self._PENALTY_RANGE[1]}]")
    ...
    def _validate_greedy_sampling(self) -> None:
        ...
        if self.best_of > 1:
            raise ValueError(f"best_of must be 1 for greedy sampling, got {self.best_of}")
        if self.top_p < 1.0 - self._SAMPLING_EPS:
...
class Request(RequestBase):
    ...
    def _check_finish_condition(self) -> None:
        ...
        if (not self.sampling_params.ignore_eos and self.generated_tokens 
