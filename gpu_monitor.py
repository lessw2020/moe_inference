...
def kill_zombie_process(pid):
    """
    Attempt to remove a zombie process.
    ...
    try:
        ...
        if process.status() == psutil.STATUS_ZOMBIE:
            # Try SIGCHLD first (signal to parent to reap zombie)
            parent = process.parent()
            if parent:
                try:
                    parent.send_signal(signal.SIGCHLD)
                    time.sleep(0.1)  # Give parent time to reap
                except psutil.NoSuchProcess:
                    pass

            # If zombie still exists, try SIGKILL
            try:
                process = psutil.Process(pid)  # Check if still exists
                if process.status() == psutil.STATUS_ZOMBIE:
        ...
        print(f"Error killing zombie process {pid}: {str(e)}")
        return False


def cleanup_zombie_processes(quiet=False):
    """
    Find and attempt to clean up all zombie processes.

    Args:
        quiet (bool): If True, suppress output

    Returns:
        tuple: (cleaned_count, failed_count, zombie_list)
    """
    zombies = []
    cleaned = 0
    failed = 0
                ...
                zombies.append(
                    {
                        ...
                        "ppid": process.ppid() if process.parent() else None,
                    }
                )
        except (psutil.NoSuchProcess, psutil.AccessDenied):
            continue
    ...
    if not quiet:
        if zombies:
            ...
            print("=" * 50)
            headers = ["PID", "Name", "Created", "Parent PID"]
            rows = [[z["pid"], z["name"], z["created"], z["ppid"]] for z in zombies]
            print(tabulate(rows, headers=headers, tablefmt="grid"))
        else:
            print("\nNo zombie processes found.")
            return 0, 0, []

    # Try to clean up zombies
    for zombie in zombies:
        pid = zombie["pid"]
        if kill_zombie_process(pid):
            cleaned += 1
            if not quiet:
                print(f"Successfully cleaned up zombie process {pid}")
        else:
            failed += 1
            if not quiet:
                print(f"Failed to clean up zombie process {pid}")

    if not quiet:
        print(f"\nZombie Cleanup Summary:")
        print(f"Total zombies found: {len(zombies)}")
        print(f"Successfully cleaned: {cleaned}")
        print(f"Failed to clean: {failed}")

    return cleaned, failed, zombies


# Add to the main argument parser:
if __name__ == "__main__":
    ...
    parser.add_argument(
        "--zombies", "-z", action="store_true", help="Show zombie processes"
    )
    parser.add_argument(
        "--clean-zombies",
        "-c",
        action="store_true",
        help="Attempt to clean up zombie processes",
    )
    parser.add_argument(
    ...
    try:
        if args.zombies or args.clean_zombies:
            # Import additional required modules
            import signal
            import time

            cleaned, failed, zombies = cleanup_zombie_processes(quiet=args.quiet)

            if not args.clean_zombies and zombies:
                print(
                    "\nTo attempt cleaning these zombie processes, run with --clean-zombies"
                )
